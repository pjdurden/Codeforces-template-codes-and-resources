...
int dp[1005][1005];
int stick[55];
int solution(int l,int r,int indl,int indr)
{
    if(l>r or indl>indr)return 0;
    if(indl==indr)return r-l;
    if(dp[l][r]!=-1)return dp[l][r];
    int temp=1000000;
    for(int i=indl;i<=indr;i++)
    {
        temp=min(temp, r-l + solution(l,stick[i],indl,i-1) + solution(stick[i],r,i+1,indr) );
    }
    return dp[l][r]=temp;
}
